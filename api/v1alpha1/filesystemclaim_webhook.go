...
func (v *FileSystemClaimValidator) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	fsc, err := convertToFileSystemClaim(obj)
	if err != nil {
		filesystemclaimlog.Error(err, "validate create", "name", fsc.Name)
		return nil, err
	}
...
func (v *FileSystemClaimValidator) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	...
	if err != nil {
		filesystemclaimlog.Error(err, "validate update old object", "name", oldFSC.Name)
		return nil, err
	}

	newFSC, err := convertToFileSystemClaim(newObj)
	if err != nil {
		filesystemclaimlog.Error(err, "validate update new object", "name", newFSC.Name)
		return nil, err
	}
	...
	timestamp := localDiskCreatedCond.LastTransitionTime.Time.Format("2006-01-02 15:04:05 MST")
	errMsg := fmt.Sprintf(
...
func (v *FileSystemClaimValidator) ValidateDelete(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	fsc, err := convertToFileSystemClaim(obj)
	if err != nil {
		filesystemclaimlog.Error(err, "validate delete", "name", fsc.Name)
		return nil, err
	}
